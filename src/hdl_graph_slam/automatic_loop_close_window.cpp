...
KeyFrameView::Ptr ini_source=graph->keyframes_view[loop_detection_source];
long ini_source_id = ini_source->lock()->id();
bool first=true;
  ...
  while (running) {
    ...
    if(ini_source_id==source_id && first)first=false;
      else if(ini_source_id==source_id && !first)
        {if (running) {
      running = false;
      loop_detection_thread.detach();
      loop_source = nullptr;
      graph->optimize();
  loop_candidates.clear();
  ...
  show_window = false;
    ...
    if (edge_inserted && optimize) {
      ...
      graph->optimize();
    }//optimisation can also be done at the very end @SR
