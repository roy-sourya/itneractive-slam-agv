...
int count_edges=0;
// add the first argument @SR
bool InteractiveGraph::merge_map_data(InteractiveGraph& previous_graph_, InteractiveGraph& graph_, const InteractiveKeyFrame::Ptr& key1, const InteractiveKeyFrame::Ptr& key2, const Eigen::Isometry3d& relative_pose) { // Eigen::Isometry3d& relative_pose
  ...
  /*
  start adding @SR
  if(previous_graph_.anchor_node) {
    previous_graph_.graph->removeEdge(previous_graph_.anchor_edge);
    previous_graph_.graph->detachVertex(previous_graph_.anchor_node);
  }
  
  for(const auto& prev_vertex_0 : previous_graph_.graph->vertices()){
  auto v = dynamic_cast<g2o::OptimizableGraph::Vertex*>(prev_vertex_0.second);
  auto new_v = dynamic_cast<g2o::OptimizableGraph::Vertex*>(factory->construct(factory->tag(v)));
  std::vector<double> est_prev;
  new_v->getEstimateData(est_prev);
  std::cout<<"************************\nThis is for the first graph:\n\n\n";
  for(int i=0;i<est_prev.size();i++){
      std::cout<<"\t"<<est_prev[i]<<" ";
      }
      std::cout<<"\n\n************************************\n\n";
  break;
